...
@onready var node_fishing_09 = $"../../Fishing/Fishing_09" as HoboFishing_09
...
func _enter_state() -> void:
	set_process(true)
	animator.play("Cast3") # if error: check node in inspector
...
func _on_state_entered(): # long cast
	if (animation_done.is_connected(fsm.change_state.bind(node_fishing_09))):
		pass
	else:
		animation_done.connect(fsm.change_state.bind(node_fishing_09))
	print("cast3 ready")
