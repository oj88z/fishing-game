...
@onready var node_fishing_04 = $"../../Fishing/Fishing_04" as HoboFishing_04
...
func _enter_state() -> void:
	set_process(true)
	animator.play("Reel_05-04") # if error: check node in inspector
...
func _on_state_entered():
	if (animation_done.is_connected(fsm.change_state.bind(node_fishing_04))):
		pass
	else:
		animation_done.connect(fsm.change_state.bind(node_fishing_04))
	print("reel_05-04 ready")
