...
@onready var node_fishing_02 = $"../../Fishing/Fishing_02" as HoboFishing_02
...
func _enter_state() -> void:
	set_process(true)
	animator.play("Reel_03-02") # if error: check node in inspector
...
func _on_state_entered():
	if (animation_done.is_connected(fsm.change_state.bind(node_fishing_02))):
		pass
	else:
		animation_done.connect(fsm.change_state.bind(node_fishing_02))
	print("reel_03-02 ready")
