...
@onready var node_fishing_05 = $"../../Fishing/Fishing_05" as HoboFishing_05
...
func _enter_state() -> void:
	set_process(true)
	animator.play("Reel_06-05") # if error: check node in inspector
...
func _on_state_entered():
	if (animation_done.is_connected(fsm.change_state.bind(node_fishing_05))):
		pass
	else:
		animation_done.connect(fsm.change_state.bind(node_fishing_05))
	print("reel_06-05 ready")
