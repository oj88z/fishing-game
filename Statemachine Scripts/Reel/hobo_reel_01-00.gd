...
@onready var node_idle = $"../../Idle/Idle" as HoboIdle
...
func _enter_state() -> void:
	set_process(true)
	animator.play("Reel_01-00") # if error: check node in inspector
...
func _on_state_entered():
	if (animation_done.is_connected(fsm.change_state.bind(node_idle))):
		pass
	else:
		animation_done.connect(fsm.change_state.bind(node_idle))
	print("reel_01-00 ready")
