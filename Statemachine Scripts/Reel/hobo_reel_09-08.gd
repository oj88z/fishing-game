...
@onready var node_fishing_08 = $"../../Fishing/Fishing_08" as HoboFishing_08
...
func _enter_state() -> void:
	set_process(true)
	animator.play("Reel_09-08") # if error: check node in inspector
...
func _on_state_entered():
	if (animation_done.is_connected(fsm.change_state.bind(node_fishing_08))):
		pass
	else:
		animation_done.connect(fsm.change_state.bind(node_fishing_08))
	print("reel_09-08 ready")
