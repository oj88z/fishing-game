...
@onready var node_fishing_07 = $"../../Fishing/Fishing_07" as HoboFishing_07
...
func _enter_state() -> void:
	set_process(true)
	animator.play("Reel_08-07") # if error: check node in inspector
...
func _on_state_entered():
	if (animation_done.is_connected(fsm.change_state.bind(node_fishing_07))):
		pass
	else:
		animation_done.connect(fsm.change_state.bind(node_fishing_07))
	print("reel_08-07 ready")
